...
class RewardOpti:
    ...
    def close_to_center(self, cte):
        self.reward += (self.config.cte_limit - (cte + self.config.cte_offset)) * self.config.cte_coef
        Logger.debug(f"reward center: ({self.config.cte_limit} - {cte + self.config.cte_offset}) * coef = [{(self.config.cte_limit - (cte + self.config.cte_offset)) * self.config.cte_coef}]")
    ...
    def sticks_and_carrots(self, infos, done):
