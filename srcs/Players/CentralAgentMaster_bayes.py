...
class CentralAgentMaster_bayes(Probe):
	agent: 		DQNAgent
	simulator: 	Simulator
	RO:			RewardOpti
	...
	def probe(self, config):
		print('Testing hyperparameters: ', config[0], config[1])
		self.config = config
		self._init_agent(self.config.config_Agent)
		self.run_remote_episode(num_frames = 10) ## TODO NUM FRAMES AND OTHER SETTINGS IN CONFIG
