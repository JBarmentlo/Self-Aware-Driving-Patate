...
try:
    ...
    input_bucket = conn.create_bucket('ED_part3-input')
    input_bucket.sync_directory('input')
    input_bucket.add_file('requirements.txt')
    input_bucket.add_file('encodDecod.py')
    ...
    output_bucket = conn.create_bucket('output_2')
    ...
    task.constants['DOCKER_CMD'] = "python3 encodDecod.py"
    ...
    task.download_results('output_2')
